...
const ReturnDetailModal: React.FC<ReturnDetailModalProps> = ({
  ...
  const handleStatusUpdate = async () => {
    ...
    if (!returnData.action || (returnData.action !== 'approve' && returnData.action !== 'reject')) {
      console.error('Invalid action type:', returnData.action);
      return;
    }
    ...
    try {
      ...
      console.log(`Attempting to ${status} return ${returnData.id}`);
      ...
      if (socket?.connected) {
        // Pastikan socket mengirim ke namespace yang benar
        socket.emit('return_status', {
          return_id: returnData.id,
          status,
          reseller_id: returnData.reseller_id
        }, {
          namespace: '/reseller' // Tambahkan namespace
        });
        console.log('WebSocket event emitted to reseller namespace');
      }
      ...
      // Tambahkan feedback error ke user jika diperlukan
    } finally {
          ...
          <>
            ...
            <ModalBody>
              <div className="space-y-6">
                ...
                <div className={`p-4 rounded-lg ${isApproval
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <Icon
                      ...
                      width={20}
                    />
                    <span className={`font-medium ${isApproval ? 'text-success' : 'text-danger'
                      }`}>
